...
orthogonal to that subspace is sent to zero, so $MM^T$ cannot act as the identity on
all of $\mathbb{R}^n$. The one-sided identity $M^TM = I_p$ is nonetheless sufficient
for our purposes: it allows us to cancel $M^TM$ to $I_p$ whenever this product appears,
which is the key cancellation exploited in Steps 1 and 3 below.
\end{result}
